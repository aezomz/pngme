...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, ChunkNotFoundError> {
        if let Some(matched_index) = self.chunks.iter().position(|c| c.chunk_type.to_string() == chunk_type)
        { Ok(self.chunks.remove(matched_index))}
        else { Err(ChunkNotFoundError{chunk_type: chunk_type.to_string() })}
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        ...
        let matched_index = self.chunks.iter().position(|c| c.chunk_type.to_string() == chunk_type);
        if let Some(matched_index) = self.chunks.iter().position(|c| c.chunk_type.to_string() == chunk_type)
            {Some(&self.chunks[matched_index])}
        else { None}
    }
...
impl From<&PathBuf> for Png {
    fn from(file: &PathBuf) -> Self {
        ...
        Png::try_from(bytes.as_slice()).expect("Not a valid PNG")
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(PngDecodeError::boxed(format!("Bad header, received {:?}, {:?}", header, Png::STANDARD_HEADER)));
        }
        // 1st 8 bytes are header signature, remaining are in 4 bytes chunk
        let mut chunks: Vec<Chunk> = Vec::new();
        let mut cur_pos: usize = 8;

        while cur_pos < bytes.len() {
            println!("bytes length: {}",bytes.len());

            let data_length = u32::from_be_bytes(<[u8; 4]>::try_from(&bytes[cur_pos..(cur_pos + 4)])?) as usize;
            println!("length: {}",data_length);
            let chunk = Chunk::try_from(&bytes[cur_pos..(cur_pos+data_length+12)]);
            println!("chunk: {:?}", chunk);
            match chunk {
                Ok(chunk) => {
                    chunks.push(chunk);
                    cur_pos += data_length + 12;
                    println!("updated_pos: {}", cur_pos);
                }
                Err(_) => return Err(PngDecodeError::boxed(format!("Error decoding chunk"))),
            }
        }
        println!("am out");
